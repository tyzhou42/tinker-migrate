...
DEFAULT_PROMPTS: dict[str, str] = {
    ...
    "reasoning_user_prompt_template": (
        ...
        "Rulebook:\n{rulebook}\n\n"
        "Text:\n{text}\n\n"
        "Return exactly in this format (no JSON object):\n"
        "<think>\n"
        "concise rule-grounded explanation\n"
        "</think>\n"
        "Then output exactly one of:\n"
        "<label>\n"
        "0\n"
    ...
    "label_only_user_prompt_template": (
        "{task_instruction}\n\n"
        "Use the rulebook below, then return only the final label line.\n\n"
        "Rulebook:\n{rulebook}\n\n"
        "Text:\n{text}\n\n"
    ...
    "student_reasoning_user_prompt_template_no_rulebook": (
        "{task_instruction}\n\n"
        "Do not use any external rulebook. Reason step-by-step based only on the text, "
        "and mimic the teacher response style shown in training data.\n\n"
        "Text:\n{text}\n\n"
        "Return exactly in this format (no JSON object):\n"
        "<think>\n"
        "concise explanation in teacher style\n"
        "</think>\n"
        "Then output exactly one of:\n"
        "<label>\n"
        "0\n"
...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "min_lr_ratio": 0.1,
  "max_length": 2048,
  # Max tokens to generate during eval/inference style generation exports.
  # This is a "max_new_tokens" limit (like OpenAI/Tinker sampling params).
  "max_new_tokens": 4096,
  "eval_interval": 10,
  # Save val per-example generation records every K eval events during student training.
  # 0 disables. (An "eval event" is each time the training loop runs val/test eval.)
  "val_export_every_evals": 0,
  "max_eval_samples": 0,
...
class PromptConfig:
    ...
    student_reasoning_user_prompt_template_no_rulebook: str


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Reasoning SFT with teacher LLM + Tinker student")
    ...
    p.add_argument("--max-new-tokens", "--max_new_tokens", dest="max_new_tokens", type=int, default=None)
    p.add_argument(
...
def resolve_path(project_root: Path, path_like: str | None) -> Path | None:
    ...
    # Support both legacy repo-root-relative paths (e.g. "tinker/rules")
    # and new tinker-dir-relative paths (e.g. "rules"), regardless of whether
    # project_root points at repo root or the tinker directory.
    if project_root.name == "tinker":
        if p.parts and p.parts[0] == "tinker":
            return project_root.parent / p
        return project_root / p

    tinker_dir = project_root / "tinker"
    if p.parts and p.parts[0] == "tinker":
        return project_root / p
...
def is_gemini_teacher_model(model_name: str | None) -> bool:
...
def load_config(project_root: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    teacher_max_tokens_arg = getattr(args, "teacher_max_tokens", None)
    teacher_temperature_arg = getattr(args, "teacher_temperature", None)
    teacher_model_name_arg = getattr(args, "teacher_model_name", None)
    ...
    max_new_tokens_arg = getattr(args, "max_new_tokens", None)
    val_export_every_evals_arg = getattr(args, "val_export_every_evals", None)
    student_use_rulebook_arg = getattr(args, "student_use_rulebook", None)
    ...
    reasoning_token_weight_arg = getattr(args, "reasoning_token_weight", None)
    label_token_weight_arg = getattr(args, "label_token_weight", None)
    ttl_seconds_arg = getattr(args, "ttl_seconds", None)
    ...
    if teacher_max_tokens_arg is not None:
        cfg["teacher_max_tokens"] = int(teacher_max_tokens_arg)
    if teacher_temperature_arg is not None:
        cfg["teacher_temperature"] = float(teacher_temperature_arg)
    if teacher_model_name_arg is not None:
        cfg["teacher_model"] = str(teacher_model_name_arg)
    ...
    if max_new_tokens_arg is not None:
        cfg["max_new_tokens"] = int(max_new_tokens_arg)
    if val_export_every_evals_arg is not None:
        cfg["val_export_every_evals"] = int(val_export_every_evals_arg)
    if student_use_rulebook_arg is not None:
        cfg["student_use_rulebook"] = bool(student_use_rulebook_arg)
    ...
    if reasoning_token_weight_arg is not None:
        cfg["reasoning_token_weight"] = float(reasoning_token_weight_arg)
    if label_token_weight_arg is not None:
        cfg["label_token_weight"] = float(label_token_weight_arg)
    if ttl_seconds_arg is not None:
        cfg["ttl_seconds"] = int(ttl_seconds_arg)
    ...
    # If user switches the teacher model to Gemini via CLI/script, make the run
    # work with sane defaults unless they explicitly configured provider details.
    if is_gemini_teacher_model(str(cfg.get("teacher_model", ""))):
        teacher_api_key_env = str(cfg.get("teacher_api_key_env", "")).strip()
        teacher_base_url = str(cfg.get("teacher_base_url", "")).strip()
        if not teacher_api_key_env or teacher_api_key_env == "DEEPSEEK_API_KEY":
            cfg["teacher_api_key_env"] = "GOOGLE_API_KEY"
        if not teacher_base_url or teacher_base_url == "https://api.deepseek.com":
            cfg["teacher_base_url"] = GEMINI_OPENAI_BASE_URL
    ...
    if int(cfg["teacher_max_tokens"]) <= 0:
        raise ValueError("teacher_max_tokens must be > 0")
    ...
    if int(cfg.get("max_new_tokens", 0)) <= 0:
        raise ValueError("max_new_tokens must be > 0")
    if int(cfg.get("val_export_every_evals", 0)) < 0:
        raise ValueError("val_export_every_evals must be >= 0")
    if int(cfg["eval_interval"]) <= 0:
        raise ValueError("eval_interval must be > 0")
    if int(cfg["eval_max_concurrency"]) <= 0:
        raise ValueError("eval_max_concurrency must be > 0")
    if float(cfg["reasoning_token_weight"]) < 0.0:
        raise ValueError("reasoning_token_weight must be >= 0")
    if float(cfg["label_token_weight"]) <= 0.0:
        raise ValueError("label_token_weight must be > 0")
    if int(cfg["ttl_seconds"]) <= 0:
...
def load_prompt_config(project_root: Path, cfg: dict[str, Any]) -> PromptConfig:
    ...
    return PromptConfig(
        ...
        student_reasoning_user_prompt_template_no_rulebook=merged["student_reasoning_user_prompt_template_no_rulebook"],
    )
...
def strip_outer_think_tags(reasoning: str) -> str:
    s = str(reasoning or "").strip()
    m = re.fullmatch(r"(?is)<think>\s*(.*?)\s*</think>", s)
    if m is None:
        return s
...
def format_reasoning_label_output(reasoning: str, label: int) -> str:
    clean_reasoning = strip_outer_think_tags(reasoning)
...
def build_label_probe_assistant_prefix(reasoning_placeholder: str) -> str:
    ...
    # Probe prefix used for token-level P(1) evaluation. Keep it in assistant voice,
    # ending immediately before the label digit so "0"/"1" logprobs are meaningful.
    return f"<think>\n{clean_reasoning}\n</think>\n<label>\n"


def parse_think_reasoning_label(raw: str) -> ParsedTeacherOutput | None:
    ...
    if not reasoning:
        return None
    # New preferred format: <label>0</label> / <label>1</label> (newlines optional)
    label_match = re.search(r"(?is)<label>\s*(0|1|yes|no|true|false)\s*</label>", raw)
    if label_match is not None:
        label = normalize_label_value(label_match.group(1))
        if label is None:
            return None
        return ParsedTeacherOutput(reasoning=reasoning, label=label, source="think_xml_label")

    # Backward-compatible fallback: "label": 0/1
    ...
    if label is None:
        return None
    return ParsedTeacherOutput(reasoning=reasoning, label=label, source="think_label")
...
def parse_teacher_output(raw: str) -> tuple[ParsedTeacherOutput | None, str | None]:
    ...
    for candidate in (raw, strip_code_fence(raw)):
        parsed = parse_think_reasoning_label(candidate)
    ...
    # If the model emits <label>...</label> but forgets <think> tags, accept it
    # when there is non-empty reasoning text we can wrap.
    match = re.search(r"(?is)<label>\s*(0|1|yes|no|true|false)\s*</label>", raw)
    if match:
        label = normalize_label_value(match.group(1))
        if label is not None:
            reasoning_txt = re.sub(re.escape(match.group(0)), "", raw, count=1).strip(" \n:-")
            reasoning = ensure_think_wrapped_reasoning(reasoning_txt)
            if reasoning:
                return ParsedTeacherOutput(reasoning=reasoning, label=label, source="regex_xml"), None
    ...
    if match:
        ...
        if label is not None:
            ...
            if not reasoning:
                reasoning = raw
            reasoning = ensure_think_wrapped_reasoning(reasoning)
            if not reasoning:
...
def build_student_reasoning_user_prompt(
    ...
    *,
    ...
    if use_rulebook:
        return build_reasoning_user_prompt(prompt_cfg, rulebook, text)
    return prompt_cfg.student_reasoning_user_prompt_template_no_rulebook.format(
        task_instruction=prompt_cfg.task_instruction,
        text=text,
    )
...
def request_teacher_sample(
    ...
    *,
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not response.choices:
                ...
                if parse_error is not None and finish_reason is not None:
                    last_error = f"{parse_error}|finish_reason={finish_reason}"
                else:
    ...
    return last_content, None, last_error
...
def build_train_examples(
    ...
    for sample in accepted_rows:
        ...
        assistant_content = format_reasoning_label_output(str(sample["reasoning"]), label)
        ...
        label_probe_assistant_prefix = build_label_probe_assistant_prefix(str(sample["reasoning"]))
        ...
        # format_reasoning_label_output() ends with "<label>\\n{0|1}\\n</label>", so label tokens
        # are a suffix of answer_tokens. Weight that whole suffix (digit + tags) so the
        # label digit is always included (the digit is not the final token anymore).
        ...
        weights = [0.0] * len(prompt_tokens) + answer_weights
        datum = tinker.Datum(
...
def build_eval_rows(
    ...
    for _, row in frame.iterrows():
        text = str(row["text"])
        label = int(row["label"])
        label_probe_assistant_prefix = build_label_probe_assistant_prefix(reasoning_placeholder)
...
def _parse_generated_label(
    ...
    m = re.search(r"(?is)<label>\s*([01])\s*</label>", generated_text)
    if m:
...
def evaluate_binary(
    ...
    def consume_record(
        ...
        if collect_examples:
            parsed_output, parse_error = parse_teacher_output(emitted_text)
            example_rows.append(
                {
    ...
    auroc = float("nan")
    auprc = float("nan")
    if bool(compute_auc) and y_true and len(set(y_true)) > 1:
        try:
...
def _save_named_sampling_client_with_ttl(
    training_client: Any,
    *,
    name: str,
    ttl_seconds: int,
) -> Any:
    """Save named sampling weights with TTL, with compatibility fallback."""
    try:
        ...
        logger.warning(
            "save_weights_and_get_sampling_client() does not accept ttl_seconds; "
            "falling back to name-only save for {}",
            name,
        )
...
def main() -> None:
    ...
    student_model_name = str(cfg.get("student_model_name", "")).lower()
    tokenizer_name_or_path = str(getattr(tokenizer, "name_or_path", "")).lower()
    enable_student_thinking = bool(cfg.get("student_enable_thinking", True)) and (
        "qwen3" in student_model_name or "qwen3" in tokenizer_name_or_path
    )
    if enable_student_thinking:
        logger.info("Enabling chat template thinking mode for Qwen3 student prompts.")
    ...
    reasoning_prompt_builder = lambda text: build_student_reasoning_user_prompt(
        prompt_cfg,
        text,
    ...
    train_rows = build_train_examples(
        ...
        enable_thinking=enable_student_thinking,
    )
    ...
    for epoch in range(int(cfg["num_epochs"])):
        ...
        for i in range(0, len(order), int(cfg["batch_size"])):
            ...
            if do_eval:
                step_client = _save_named_sampling_client_with_ttl(
                    training_client,
    ...
    final_client = _save_named_sampling_client_with_ttl(
        training_client,
