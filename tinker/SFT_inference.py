...
def _find_reusable_reasoning_artifacts(
    *,
    student_dir: Path,
    split_name: str,
    checkpoint_path: str,
    ...
    """Find reusable predictions/metrics files for a split and selected checkpoint."""
    if split_name == "test":
        candidates = [
            (student_dir / "test.json", student_dir / "metrics_test.json"),
            (student_dir / "test_bestval.json", student_dir / "metrics_test_bestval.json"),
            (student_dir / "test_final.json", student_dir / "metrics_test_final.json"),
        ]
    else:
        candidates = [(student_dir / f"{split_name}.json", student_dir / f"metrics_{split_name}.json")]

    for preds_path, metrics_path in candidates:
        if not preds_path.exists():
            continue
        if _artifact_matches(metrics_path, checkpoint_path=checkpoint_path, fingerprints=fingerprints):
...
def _load_model_pointer(project_root: Path, run_name: str) -> dict[str, Any] | None:
    """
    Load a model pointer JSON from model/.

    Resolution order:
      1) model/<run_name>.json
      2) model/latest.json
    """
...
def _build_label_only_eval_rows(
    *,
    ...
    for _, row in df.iterrows():
        text = str(row["text"])
        label = int(row["label"])
        # Fit prompt to max_length by reserving a tiny dummy answer in the target format.
        ...
        # Probe P(1)/P(0) for the digit conditioned on the model already emitting "<label>".
        label_open = tokenizer.encode("<label>\n", add_special_tokens=False)
        if not label_open or len(prompt_tokens) + len(label_open) > max_length:
            dropped += 1
            continue
        prompt_tokens = prompt_tokens + label_open
...
def main() -> None:
    ...
    # Inference generation params are CLI-driven; store them in cfg so fingerprints
    # and reuse checks reflect generation behavior.
    cfg["max_new_tokens"] = int(getattr(args, "max_new_tokens", cfg.get("max_new_tokens", 512)))
    ...
    if do_reasoning:
        ...
        # Compute threshold lazily only if we need fresh eval/generation.
        decision_threshold: float | None = None
        reused_threshold: float | None = None

        for split_name in ("train", "val", "test"):
            out_path = student_dir / f"{split_name}.json"
            metrics_path = student_dir / f"metrics_{split_name}.json"

            reusable = None if args.force else _find_reusable_reasoning_artifacts(
            ...
            if reusable is not None:
                reuse_out_path, reuse_metrics_path = reusable
                metrics_obj = json.loads(reuse_metrics_path.read_text(encoding="utf-8"))
                metrics = metrics_obj.get("metrics") or {}
                if reused_threshold is None:
                    thr_obj = metrics_obj.get("decision_threshold")
                    if thr_obj is None:
                        thr_obj = metrics.get("decision_threshold")
                    if thr_obj is not None:
                        try:
                            reused_threshold = float(thr_obj)
                        except Exception:
                            pass
                _print_metrics(split_name, metrics)
                manifest["outputs"][split_name] = {
                    "reused": True,
                    "predictions_path": str(reuse_out_path),
                    "metrics_path": str(reuse_metrics_path),
                }
                continue
        ...
        if decision_threshold is None and reused_threshold is not None:
            manifest["decision_threshold"] = float(reused_threshold)
            (student_dir / "manifest.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False), encoding="utf-8")
        elif "decision_threshold" not in manifest:
            manifest["decision_threshold"] = 0.5
            (student_dir / "manifest.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False), encoding="utf-8")
