...
RUN_NAME="${RUN_NAME:-reasoning_sft_$(date +%Y%m%d_%H%M%S)}"
SOURCE_RUN_NAME="${SOURCE_RUN_NAME:-reasoning_sft_20260301_114319}"
DATASET_NAME="${DATASET_NAME:-ethos}"
MODEL_NAME="${MODEL_NAME:-Qwen/Qwen3-8B}"
WANDB_PROJECT="${WANDB_PROJECT:-reasoning-sft-gpu}"
WANDB_MODE="${WANDB_MODE:-online}"
ENABLE_STAGE1="${ENABLE_STAGE1:-true}"
ENABLE_STAGE2="${ENABLE_STAGE2:-true}"
STUDENT_USE_RULEBOOK="${STUDENT_USE_RULEBOOK:-true}"
...
echo "======================================="
echo "GPU Reasoning SFT"
echo "======================================="
echo "Run name          : ${RUN_NAME}"
echo "Teacher source run: ${SOURCE_RUN_NAME}"
...
if [[ "${ENABLE_STAGE1}" == "true" ]]; then
  "${PYTHON_BIN}" "${SCRIPT_DIR}/reasoning_teacher_prep.py" \
    --run-name "${RUN_NAME}" \
    --source-run-name "${SOURCE_RUN_NAME}" \
    --dataset-name "${DATASET_NAME}"
fi

if [[ "${ENABLE_STAGE2}" == "true" ]]; then
  RULEBOOK_FLAG="--no-student-use-rulebook"
  if [[ "${STUDENT_USE_RULEBOOK}" == "true" ]]; then
    RULEBOOK_FLAG="--student-use-rulebook"
  fi

  "${ACCELERATE_BIN}" \
    "${LAUNCH_ARGS[@]}" \
    "${SCRIPT_DIR}/reasoning_sft.py" \
    --run-name "${RUN_NAME}" \
    --source-run-name "${SOURCE_RUN_NAME}" \
    --dataset-name "${DATASET_NAME}" \
